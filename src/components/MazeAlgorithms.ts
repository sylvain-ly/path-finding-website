...
function pickOrientation(
  ...
  if (width > height + 1) {
    return 'V';
  } // plus large que haut → mur vertical
  if (height > width + 1) {
    return 'H';
  } // plus haut que large → mur horizontal
...
function buildRecursiveDivisionStepsSkew(
  ...
  for (let c = 0; c < cols; c++) {
    if (!isStartOrEnd(0, c, start, end)) {
      steps.push([0, c]);
    }
    if (!isStartOrEnd(rows - 1, c, start, end)) {
      steps.push([rows - 1, c]);
    }
  }
  for (let r = 1; r < rows - 1; r++) {
    if (!isStartOrEnd(r, 0, start, end)) {
      steps.push([r, 0]);
    }
    if (!isStartOrEnd(r, cols - 1, start, end)) {
      steps.push([r, cols - 1]);
    }
  }
  ...
  const divide = (r0: number, c0: number, r1: number, c1: number) => {
    ...
    if (orientation === 'H') {
      ...
      for (let c = c0; c <= c1; c++) {
        ...
        if (!isStartOrEnd(wallRow, c, start, end)) {
          steps.push([wallRow, c]);
        }
      }
      ...
      for (let r = r0; r <= r1; r++) {
        ...
        if (!isStartOrEnd(r, wallCol, start, end)) {
          steps.push([r, wallCol]);
        }
      }
